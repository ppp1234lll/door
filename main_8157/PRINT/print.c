...
//		if((str = strstr((char *)print_buff,DEVICE_PORT1)) != NULL) 
//		{
//			str = strstr((char *)str,":");
//			sscanf((char*)str,"%*[^\"]\"%[^\"]",id_str); 
//			print_param.port1 = strtol(id_str, NULL, 16);
//		}
//		
//		if((str = strstr((char *)print_buff,DEVICE_PORT2)) != NULL) 
//		{
//			str = strstr((char *)str,":");
//			sscanf((char*)str,"%*[^\"]\"%[^\"]",id_str); 
//			print_param.port2 = strtol(id_str, NULL, 16);
//		}
...
//		/* 主网检测IP2 */
	...
	my_cjson_join_string_function(pdata,(uint8_t*)"pwd",(uint8_t*)temp,1);
...
void print_tcp_configure_mac(void)
{
	...
	/* 保存 */
	STMFLASH_Write_SAVE(DEVICE_ID_ADDR,(u16*)device->id.c,4);	
...
void printf_tcp_configure_pwd_name(void)
{
  ...
  memcpy(param.name,print_param.username,sizeof(print_param.username));
	...
	app_set_code_name_function( param);
