...
#define SAVE_COMPARISION_PARAMETER ("comparision_param")  /* 外设控制参数：温度控制、摄像头IP */
...
#define SAVE_REPORT_SWITCH_NAME     ("report_switch")     /* 上报开关状态 */
...
#define SAVE_UPDATE_FILE_NAME       ("updateaddr")        /* 更新地址 */
/************************************************************
*
...
void save_read_default_local_network(struct local_ip_t *local)
{
	...
	/* 本机MAC */
	STMFLASH_Read(DEVICE_MAC_ADDR, (u16 *)mac, sizeof(mac) / 2);
	...
	memset(local->ping_ip,0,sizeof(local->ip));
...
void save_read_default_remote_ip(struct remote_ip *remote)
{
	...
	sprintf((char*)remote->outside_iporname,"test1.fnwlw.net");
	remote->outside_port  = 6102;
	
	memset(remote->inside_iporname,0,sizeof(remote->inside_iporname));
	sprintf((char*)remote->inside_iporname,"test1.fnwlw.net");
	remote->inside_port  = 6102;
...
void save_read_default_device_paramter_function(struct device_param *param)
{
	...
	STMFLASH_Read(DEVICE_ID_ADDR,(u16*)data_id.c,4);
	...
	sprintf((char*)param->name,DEFALUT_USERNAME);
	sprintf((char*)param->password,DEFALUT_PASSWORD);
...
int8_t save_read_update_addr(uint8_t *ip,uint32_t *port)
{
	...
	else
	{
		...
		ip[3] = 214;
		*port = 8989;
		save_stroage_update_addr(ip,*port);
		ret = -1;
	}
	err = lfs_file_close(&g_lfs_t, &lfs_fp);
...
void save_read_default_backups(sys_backups_t *param)
{
	...
	sprintf((char*)param->remote.outside_iporname,"test1.fnwlw.net");
	param->remote.outside_port  = 6102;
	
	memset(param->remote.inside_iporname,0,sizeof(param->remote.inside_iporname));
	sprintf((char*)param->remote.inside_iporname,"test1.fnwlw.net");
...
int8_t save_read_http_ota_function(struct update_addr *param)
{
	...
	if(SAVE_DEBUG)  printf("save_read_http_ota_function:%d\r\n",err);
	if(err == 0)
	{
		err = lfs_file_rewind(&g_lfs_t, &lfs_fp);
...
void save_read_default_http_ota(struct update_addr *param)
{
	...
	strcpy((char*)param->update_url,"47.104.98.214");
	param->update_port = 8989;
}
