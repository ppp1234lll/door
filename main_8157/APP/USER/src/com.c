...
void com_report_normally_function(uint8_t *data, uint16_t *len, uint8_t cmd)
{
  ...
  uint32_t			dtemp   = 0;
	...
	#ifdef COMDATA_PROCESS_MODE1
	sprintf((char*)str,"QN=%s;",app_get_report_current_time(1));
...
//																	//com_report_get_adapter_status(1));
//	strcat((char*)data,(char*)str);
	/** 摄像机的网络状态 **/
	memset(str,0,sizeof(str));
	...
	temp = det_get_vin220v_handler(3);
	...
	/** 继电器状态 **/
	memset(str,0,sizeof(str));
	sprintf((char*)str,"RELAY=%01d,%01d;",
	...
	if(relay_get_status_function(RELAY_1) == 1)
		Vin220_Handler(str_buff[0],0);
	else
		sprintf((char*)str_buff[0],"%d",0);

	if(relay_get_status_function(RELAY_2) == 1)
		Vin220_Handler(str_buff[1],0);
	else
		sprintf((char*)str_buff[1],"%d",0);	
	
	sprintf((char*)str,"CHV=%s,%s;",str_buff[0],str_buff[1]);
	strcat((char*)data,(char*)str);
	
	/** 各通道电流 **/
	memset(str,0,sizeof(str));
	memset(str_buff,0,sizeof(str_buff));
	Vin220_Handler(str_buff[0],1);
  Vin220_Handler(str_buff[1],2);
	sprintf((char*)str,"CHA=%s,%s;",str_buff[0],str_buff[1]);
	strcat((char*)data,(char*)str);	
  ...
  /* 传输模式 */
	...
	for(index=0;index<10;index++)
	{
		...
		sprintf(temp,"%02x-%02x-%02x-%02x-%02x-%02x", 
						comparision->mac[index][0],comparision->mac[index][1],\
						comparision->mac[index][2],comparision->mac[index][3],\
						comparision->mac[index][4],comparision->mac[index][5]);
		sprintf((char*)name,"m%d",index+1);
		my_cjson_join_string_function(pdata,name,(uint8_t*)temp,1);
	}
	
	/* 过压、欠压、电流 */
	memset(temp,0,sizeof(temp));
	sprintf(temp,"%d,%d,%d",threshol->volt_max,threshol->volt_min,threshol->current);
...
//	printf("com_heart_pack_function:");
...
void com_ipc_ip_information(uint8_t *pdata, uint16_t *size)
{
	...
	if(json != NULL && data != NULL)
	{
		...
		sprintf(crc_buff,"%08d%09d",sg_comqn_t.qn1,sg_comqn_t.qn2); 	/* 添加QN */
		...
		cJSON_AddStringToObject(data,"ver","10");   // 数据版本
		memset(temp,0,sizeof(temp));
		sprintf(temp,"%d",device->id.i);              
		cJSON_AddStringToObject(data,"tid",temp);   // 设备IP
		cJSON_AddStringToObject(data,"cmd","E4");   // 命令
		
		
		for(uint8_t i=0; i< num;i++)
		{
			memset(buff,0,sizeof(buff));
			onvif_get_ipc_appoint_ip(buff,i);
			sprintf((char*)name,"ip%d",i+1);
			cJSON_AddStringToObject(data,name,buff);   // IP地址
		}
		memset(temp,0,sizeof(temp));
		sprintf(temp,"%x",crc);
		cJSON_AddStringToObject(data,"crc",temp);
		
		json_data =  (uint8_t*)cJSON_PrintUnformatted(json); // 生成JSON数据 - 使用后需要释放内存
	}	
	cJSON_Delete(json);
	cJSON_Delete(data);
	...
	*size = strlen((char*)json_data);
	...
	if(ONVIF_DEBUG) printf("com_ipc_ip_information \r\n");
}
/************************************************************
*
...
*	              20220329
************************************************************/
void com_ipc_device_information(uint8_t *pdata, uint16_t *size)
{
...
int8_t com_deal_configure_server_domain_name(com_rec_data_t *buff)
{
	...
	if (p1 != NULL) {
		memset(ota_p->update_url,0,sizeof(ota_p->update_url));
		p1 += 8;
		sscanf((char*)p1,"%[^:]:%d",ota_p->update_url,&ota_p->update_port);
		app_set_http_ota_function(*ota_p);
		goto EXIT;
	}
...
//		update_set_update_mode(UPDATE_MODE_GPRS);
		if(buff->buff[0] == UPDATE_MODE_GPRS)
		update_set_update_mode(UPDATE_MODE_GPRS);
		if(buff->buff[0] == UPDATE_MODE_LWIP)
...
void com_deal_configure_mac(com_rec_data_t *buff)
{
	...
	/* 保存 */
	STMFLASH_Write_SAVE(DEVICE_MAC_ADDR, (u16 *)&local->mac, sizeof(local->mac) / 2);
	app_set_save_infor_function(SAVE_LOCAL_NETWORK);
	eth_set_network_reset();
...
{
	eth_set_udp_onvif_flag(2);
	if( app_get_carema_search_mode() == 1 ) // 判断摄像机搜索协议
	{
		if(ONVIF_SEARCH_DEBUG) printf("com_deal_configure_onvif_carema\r\n");
...
void com_deal_camera_config(com_rec_data_t *buff)
{
	...
	if( ip[0] == 0 &&ip[1] == 0 &&ip[2] == 0 &&ip[3] == 0)
	{
		...
		app_set_camera_mac_function(mac,num);
		app_set_reply_parameters_function(buff->cmd,0x01);
	}
	else
	{
...
{	
	struct threshold_params param;
	
	param.volt_max 	= (buff->buff[0]<<8|buff->buff[1]); // 前2个字节
	param.volt_min  = (buff->buff[2]<<8|buff->buff[3]); // 前2个字节
	param.current  	= (buff->buff[4]<<8|buff->buff[5]); // 前2个字节
...
//		printf("com_queue_find_msg\n");
		...
		switch(recdata_t.cmd)
		{
			...
			case CONFIGURE_SET_MAC: 			// 设置MAC
				...
				break;
			case CONFIGURE_CAMERA_CONFIG:		// 设置摄像头信息
			...
			case CONFIGURE_FAN_HUMI:			// 设置风扇与湿度
...
/*********************************************************
				数据缓存区
**********************************************************/
static com_queue_t sg_comqueue_t = {0};
