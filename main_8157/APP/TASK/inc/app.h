...
struct remote_ip				
{
	...
	uint8_t  inside_iporname[128];	  // 内网IP-域名
...
struct update_addr            
{   
    uint8_t  update_url[128];     // 升级服务器IP或URL（字符串，最长127字符）
    uint32_t update_port;         // 升级服务器端口号
};
...
typedef struct
{
	...
	struct update_addr ota;          // OTA升级配置（对应 update_addr 结构体）
	uint8_t mem;			   	 // 内存利用率
} sys_param_t;
...
void app_set_current_time(int *time);											// 设置当前时间
...
// 设置HTTP OTA升级参数（param：升级参数，对应 update_addr 结构体）
