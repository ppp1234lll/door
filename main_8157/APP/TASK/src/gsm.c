...
uint8_t  *sg_multi_buff;
uint16_t sg_multi_flag = 0;
uint8_t  *sg_multi2_buff;
uint16_t sg_multi2_flag = 0;
uint8_t  sg_multi_link = 0;
...
void gsm_tcp_control_function(void)
{
	...
	if(sg_gsmoperate_t.tcp_status == 0) 
	{
		...
		if(flag == 0) {
			/* 进行状态监测 */
			if(GPRS_DEBUG)  printf("gprs_network_status_monitoring_function..2...\n ");
			if(gprs_network_status_monitoring_function() != 0) 
			{
				/* 网络注册有问题,需要重启模块*/
				gprs_module_restart_function();
...
/************************************************************
*
* Function name	: gsm_network_status_check
* Description	: 无线网络状态检测
* Parameter		: 
* Return		: 
*	
************************************************************/
...
{
	static uint16_t gsm_cnt  = 0;
	static uint16_t gsm_net_error  = 0;
	
	if(sg_gsmoperate_t.tcp_status == 1)
	{
		gsm_cnt++;
		if(gsm_cnt >= 1000) // 10s
		{
			gsm_cnt = 0;
			if(gprs_tcplink_status_function()<0)  // 查询TCP连接状态
			{
				gsm_net_error++;
			}
			else
