...
void eth_network_line_status_detection_function(void)
{
	...
	while(1)
	{
		...
		if( app_get_carema_search_mode() == 1 ) // 判断摄像机搜索协议
		{
			...
			if(lwipdev.udp_onvif_flag == 0)  // 开启搜索命令后，不删除onvif任务
			{
				if(lwipdev.udp_status != LWIP_UDP_NO_CONNECT) 
				{
		...
		{
			if(lwipdev.udp_status == LWIP_UDP_NO_CONNECT) // 检测到UDP还未连接
			{
...
int8_t eth_network_line_status_check(void)
{
  ...
  static uint8_t count           = 0;
	...
	if(lwipdev.reset == 1) 
	{
		...
		if (lwipdev.udp_status != LWIP_UDP_NO_CONNECT)   // 重启ONVIF连接
		{
			eth_set_onvif_udp_connect_reset();
		}
