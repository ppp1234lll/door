...
/* 控制IO */
#define GPRS_NRST_GPIO_CLK		RCC_APB2Periph_GPIOE
#define GPRS_NRST_GPIO 				GPIOE
#define GPRS_NRST_PIN  				GPIO_Pin_10

#define GPRS_PWRK_GPIO_CLK		RCC_APB2Periph_GPIOE
#define GPRS_PWRK_GPIO 				GPIOE
#define GPRS_PWRK_PIN 				GPIO_Pin_12

#define GPRS_PWR_GPIO_CLK		  RCC_APB2Periph_GPIOE
#define GPRS_PWR_GPIO 				GPIOE
#define GPRS_PWR_PIN 				  GPIO_Pin_7

#define GPRS_Sel_GPIO_CLK		  RCC_APB2Periph_GPIOE
#define GPRS_Sel_GPIO 				GPIOE
#define GPRS_Sel_PIN 				  GPIO_Pin_15
...
#define GPRS_PWR_H GPIO_WriteBit(GPRS_PWR_GPIO,GPRS_PWR_PIN,Bit_SET)
#define GPRS_PWR_L GPIO_WriteBit(GPRS_PWR_GPIO,GPRS_PWR_PIN,Bit_RESET)
...
void gprs_gpio_init_function(void)
{
	...
	RCC_APB2PeriphClockCmd(GPRS_NRST_GPIO_CLK|GPRS_PWRK_GPIO_CLK,ENABLE);
	...
	GPIO_Init(GPRS_PWR_GPIO,&GPIO_InitStructure);
  ...
  uint8_t addr_len = 0;
	...
	switch(sg_gprs_status_t.step) {
		case 0:
			repeat = 0;	/* 数据清零 */
			init_repeat = 0;
			gprs_boot_up_function();		/* 设备开机 */
			sg_gprs_status_t.step = 1;
		
			gsm_error_cnt = STMFLASH_ReadHalfWord(GSM_ERROR_ADDR); // 读取数据
			if(gsm_error_cnt > 10)
				gsm_error_cnt = 0;
			break;
		case 1:
			...
			repeat = 0;
			if(gsm_error >= 3) /* 初始化失败3次 */
			{			
				gsm_error = 0;
				gsm_error_cnt++;
				Test_Write(GSM_ERROR_ADDR,gsm_error_cnt); // 存储
				if(gsm_error_cnt <= 4)
				{
...
/************************************************************
*
* Function name	: gprs_tcplink_status_function
* Description	: TCP连接监测函数
* Parameter		: 
* Return		: 
*	
************************************************************/
int8_t gprs_tcplink_status_function(void)
{
	...
	memset(gprs_rx_buff,0,sizeof(gprs_rx_buff));
	if(gprs_send_cmd_function((uint8_t*)"AT+MIPSTATE=1\r\n",(uint8_t*)"+MIPSTATE:",50) == 0) 
	{
		p1 = (uint8_t*)strstr((char*)gprs_rx_buff,"CONNECTED");
		if(p1 != NULL)
			return 0;
	}
...
void gprs_get_receive_data_function(uint8_t *buff, uint16_t len)
{
	...
	(
	 ...
	 || ( (update_detection_status_function() == UPDATE_MODE_GPRS) && (gsm_get_network_connect_status_function() == 1 ) ) // 无线更新时
	)
	{
		// 防止溢出
