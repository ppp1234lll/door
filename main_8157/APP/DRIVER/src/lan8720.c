...
/************************************************************
*
* Function name	: LAN8720_Get_Network_Cable_Status
* Description	: 获取无线状态：是否接入网线
* Parameter		: 
* Return		: 
*	
************************************************************/
uint8_t LAN8720_Get_Network_Cable_Status(void)
{
	return (ETH_ReadPHYRegister(LAN8720_PHY_ADDRESS, PHY_BSR) & 4);
}
...
u8 ETH_MACDMA_Config(void)
{
	...
	ETH_InitStructure.ETH_CarrierSense = ETH_CarrierSense_Disable;	//GD 半双工模式下修改
	///网络MAC参数设置 
...
void ETH_IRQHandler(void)
{
	...
	while(ETH_GetRxPktSize()!=0) 	//检测是否收到数据包
	{ 
		lwip_pkt_handle();		
	}
